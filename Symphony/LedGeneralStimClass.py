...
class LedGeneralStim( AbstractProtocol):
    '''Class for LedGeneralStim analysis'''

    def __init__( self, epoch_group_object, block_uuid, count):
        super( LedGeneralStim, self).__init__( epoch_group_object, block_uuid, count)
        self.name = 'LedGeneralStim'
    ...
    def load_data( self):
        ...
        if len( self.child_list) == 1:
            ...
            led = "led%d" % self["ledChannel"]
            self.stimulus = epoch.fetch_light_stimulus(led)
    ...
    def plot_stimulus( self, ax):
        ...
        ax.plot( self.time, self.stimulus, lw=1.0, color=color)
