...
class GapFreeBoth( AbstractProtocol):
    '''Class for GapFreeBoth analysis'''

    def __init__( self, epoch_group_object, block_uuid, count):
        super( GapFreeBoth, self).__init__( epoch_group_object, block_uuid, count)
        self.name = 'GapFreeBoth'


    def load_data( self):
        '''Load data into array'''
        response_1 = np.zeros( 0)
        response_2 = np.zeros( 0)

        # Might need to concatenate here
        for epoch_no, epoch in enumerate( self.child_list):
            temp_response = epoch.fetch_response()
            response_1 = np.append( response_1, epoch.fetch_response( 'Amp1'))
            response_2 = np.append( response_2, epoch.fetch_response( 'Amp2'))
