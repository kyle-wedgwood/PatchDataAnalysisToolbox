...
class LedStepFamily( AbstractProtocol):
    '''Class for LedPulseFamily protocol analysis'''

    def __init__( self, epoch_group_object, block_uuid, count):
        super( LedStepFamily, self).__init__( epoch_group_object, block_uuid, count)
        self.name = 'LedStepFamily'
    ...
    def load_data( self):
        ...
        amplitude_ref = np.array( range( \
            self['pulsesInFamily']))*self['amplitudeIncrementPerPuse'] + \
            self['firstPulseAmplitude']
        ...
        for pulseFamily in range(self['pulsesInFamily']):
            ind = (self.time > self['preTime']) & \
                  (self.time < self['preTime'] + \
                   self['firstPulseDuration'] + \
                   pulseFamily*self['durationIncrementPerPulse'])

            stimulus[ind,pulseFamily] = self['firstPulseAmplitude'] + pulseFamily*self['amplitudeIncrementPerPuse']
        ...
        for epoch_no, epoch in enumerate( list( self.child_list)):
            ...
            pulse_time = stim_pars['pulseDuration']
            pulse_amp = stim_pars['pulseAmplitude']
            pulseFamily = np.argmin( (duration_ref - pulse_time)**2 + \
                                     (amplitude_ref - pulse_amp)**2)
    ...
    def fetch_total_time( self):
        ...
        return self['preTime'] + \
               self['firstPulseDuration'] + \
               (self['pulsesInFamily']-1)*self['durationIncrementPerPulse'] + \
               self['tailTime']
    ...
    def plot_stimulus( self, ax):
        ...
        for i in range( self['pulsesInFamily']):
            ax.plot(self.time, self.stimulus[:,i], lw=1.0, \
                    alpha=(float(i)+1.0)/(self['pulsesInFamily']+1), color=color)
