...
from propertyExcludeList import exclude_list
...
class AbstractProtocol( Base):
    ...
    def __init__( self, epoch_group_object, block_uuid, count):
        ...
        self.operating_mode = self.amp_prop.get( 'operatingMode').decode()
        ...
        for key, val in self.__dict__.items():
            if key not in exclude_list:
                self.decode_byte_string(val)
                self.pdf_vars[key] = val
    ...
    def fetch_stimulus_pars( self, epoch_uuid):
        ...
        for key, val in list(protocol_pars.items()):
            stim_pars[key] = val
    ...
    def fetch_amplifier_properties( self, amp=1):
        ...
        amp_uuid = list(response_group.items())[amp-1][0]
    ...
    def add_title(ax, plot_pars):
        string = ''
        values = ()
        for key, val in list(plot_pars.items()):
            string += key + ': %s, '
            values = values + (val,)
    ...
    def make_pdf_report( self, pdf):
        ...
        if not hasattr(self, 'exclude_list'):
            self.exclude_list = ['exclude_list']

        for key, val in sorted(list(self.pdf_vars.items())):
            if not key in self.exclude_list:
                self.decode_byte_string(val)
    ...
    def save_data_ascii( self, folder_name='.'):
        ...
        if 'stimulus' not in self.__dict__: # THIS IS A HACK 24.2.2020
            self.stimulus = np.zeros( shape=self.response.shape)

        if (type(self.stimulus) == list):
            #  Work out maximum number of rows for data structure
            max_pts = 0
            for stim_data in self.stimulus:
                max_pts = np.max([max_pts,len(stim_data)])
                
            stimulus = np.zeros(shape=(max_pts, len(self.stimulus)))
            response = np.zeros(shape=(max_pts, len(self.stimulus)))
                
            for i,(stim_data,resp_data) in enumerate(zip(self.stimulus, self.response)):
                                
                stim_data = np.append(stim_data, np.nan*np.ones(shape=(max_pts-len(stim_data))))
                resp_data = np.append(resp_data, np.nan*np.ones(shape=(max_pts-len(resp_data))))
                                
                stimulus[:,i] = stim_data
                response[:,i] = resp_data
                
            buf = np.nan*np.ones( shape=( max_pts,1))
            data = np.concatenate( (stimulus, buf, response), axis=1)
            
        elif (self.stimulus.ndim > 1):
            buf = np.nan*np.ones( shape=( self.stimulus.shape[0],1))
            data = np.concatenate( (self.time, buf, self.stimulus, buf, self.response), axis=1)

        else:
            buf = np.nan*np.ones( shape=self.stimulus.shape)
            data = np.stack( (self.time, buf, self.stimulus, buf, self.response)).transpose()
