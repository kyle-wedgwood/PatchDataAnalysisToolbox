...
class OptoBlueStim( AbstractProtocol):
    '''Class for OptoBlueStim protocol analysis'''

    def __init__( self, epoch_group_object, block_uuid, count):
        super( OptoBlueStim, self).__init__( epoch_group_object, block_uuid, count)
        self.name = 'OptoBlueStim'
        ...
        for pulseFamily in range(self['pulsesInFamily']):
            ind = (self.time > self['preTime']) & \
                  (self.time < self['preTime'] + \
                   self['firstPulseDuration'] + \
                   pulseFamily*self['incrementPerPulse'])

            stimulus[ind,pulseFamily] = 1.0
    ...
    def fetch_total_time( self):
        ...
        return self['preTime'] + \
               self['firstPulseDuration'] + \
               (self['pulsesInFamily']-1)*self['incrementPerPulse'] + \
               self['tailTime']
