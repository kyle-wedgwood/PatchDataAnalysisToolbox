function fig = plotGJSimulPulse( rawData, stimData, sampleRate, holdingValue)
    ...
    % Clean stimulus data if required
    for channel_no = 1:no_channels
      no_data_pts = size( rawData{no_channels}, 1);
      stimData.(stim_names{channel_no}) = stimData.(stim_names{channel_no})(1:no_data_pts,:);
    end
    ...
    for channel_no = 1:no_channels
      ...
      for sweep_no = 1:no_sweeps
        stimTim(channel_no) = plot( ax1, time, stimData.(stim_names{channel_no})(:,sweep_no), ...
          'Linewidth', 2.0, 'Color', c_map{channel_no}(end,:));
