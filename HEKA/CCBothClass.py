...
class CCBoth( Series):
    ...
    def __init__( self, exp_object, obj, index):
        ...
        super( CCBoth, self).__init__( exp_object, obj, index)
    ...
    def plot_stimuli_and_responses( self, protocol='all', folder_name='.'):
        ...
        if (protocol == 'all' or protocol == self.name):
            ...
            plt.plot( self.time, 1e3*self.data['V-mon'], lw=2, color='steelblue')
            plt.plot( self.time, 1e3*self.data['Adc-1'], lw=2, color='sienna')
